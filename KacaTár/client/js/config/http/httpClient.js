...
export class httpClient{
    ...
    login(data,callback){
        const url = "http://localhost/KacaT%C3%A1r/server/Controller/UserController.php";//átkell írni 
        this.fetchUser.post(url,data,(err,result)=>{
            if (err) {
                console.log("nem sikerült a fetch eküldése");
                callback(true,null)
    ...
    getProductBySearchData(data,callback){
        const url = "http://localhost/KacaT%C3%A1r/server/Controller/ProductController.php";
        this.fetchUser.post(url,data,(err,result)=>{
            console.log("fut");
            if (err) {
